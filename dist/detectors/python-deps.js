...
const HIGH_CAPABILITY_PY_DEPS = new Set([
...
function readManifestDeps(file, text) {
    if (!text.trim()) {
        return [];
    }
...
function readRequirementsTxtDeps(text) {
    const deps = [];
...
function readPyprojectDeps(text) {
    let parsed;
    try {
...
function pushPoetrySection(deps, text, tablePath, table) {
    if (!isRecord(table)) {
        return;
    }
...
function lineOfPep508InProject(text, _tableKey, name) {
