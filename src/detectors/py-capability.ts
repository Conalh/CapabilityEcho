...
function nextPyLines(added: AddedLine, sameFile: AddedLine[]): AddedLine[] {
  const out: AddedLine[] = [];
...
function isPyHighLevelNetwork(content: string): boolean {
...
function isPyLowLevelNetwork(content: string): boolean {
...
function detectPySecretExfil(
  ...
  const hasSecret = chunk.some(
    (line) => referencesPyEnvSecret(line.content) || referencesSecretVariable(line.content, secretVariables)
  );
  if (!hasSecret) {
    return [];
  }
...
function isPyExternalRequest(added: AddedLine, sameFile: AddedLine[]): boolean {
  if (!isPyHighLevelNetwork(added.content)) {
    return false;
  }
