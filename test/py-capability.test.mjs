...
test('py: requests.get without literal URL does not over-fire', () => {
  ...
  assert.equal(findings.find((f) => f.kind === 'external_fetch_added'), undefined);
});
...
test('py: subprocess.Popen flags as high', () => {
  ...
  const f = findings.find((finding) => finding.kind === 'subprocess_spawn_added');
  assert.ok(f);
...
test('py: os.system flags subprocess spawn', () => {
  ...
  assert.ok(findings.find((f) => f.kind === 'subprocess_spawn_added'));
});
...
test('py: eval() flags as critical dynamic exec', () => {
  ...
  const f = findings.find((finding) => finding.kind === 'dynamic_eval_added');
  assert.ok(f);
...
test('py: importlib.import_module flags as dynamic exec', () => {
  ...
  assert.ok(findings.find((f) => f.kind === 'dynamic_eval_added'));
});
...
test('py: pickle.loads flags as unsafe deserialize', () => {
  ...
  const f = findings.find((finding) => finding.kind === 'unsafe_deserialize_added');
  assert.ok(f);
  assert.equal(f.severity, 'critical');
});
...
test('py: yaml.load without SafeLoader flags; yaml.safe_load does not', () => {
  ...
  assert.ok(unsafe.find((f) => f.kind === 'unsafe_deserialize_added'));

  const safe = detectPyCapability([
    line('agent.py', 'config = yaml.safe_load(open("config.yml"))')
  ]);
  assert.equal(safe.find((f) => f.kind === 'unsafe_deserialize_added'), undefined);
});
